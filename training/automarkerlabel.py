...
def import_labelled_c3ds(filelist,markers,alignMkR,alignMkL,windowSize):
    ...
    #---------------------------------------------------------------------------------------
        ...
        if 'data' not in c3ddat or 'points' not in c3ddat['data']:
            print(f"Warning: No point data found in file {trial}, skipping.")
            continue

        point_data = c3ddat['data']['points']
        if point_data.size == 0 or point_data.shape[1] == 0:
            print(f"Warning: File {trial} contains no markers, skipping.")
            continue

        if point_data.shape[2] == 0:
            print(f"Warning: File {trial} contains no frames, skipping.")
            continue
    #--------------------------------------------------------------------------------------
